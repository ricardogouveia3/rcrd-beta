...
export default function HeaderToggleButton({ onClick, icon: Icon }: Readonly<HeaderToggleButtonProps>) {
  return (
    <button
      className={`flex items-center justify-center h-full w-full bg-white dark:bg-quartz-200 border-quartz-900 dark:border-quartz-450 hover:border-quartz-950 hover:dark:border-quartz-400 hover:bg-quartz-800 hover:dark:bg-quartz-150 transition-colors rounded-lg sm:rounded-full text-black dark:text-white font-semibold px-3 py-1 border`}
      onClick={onClick}
